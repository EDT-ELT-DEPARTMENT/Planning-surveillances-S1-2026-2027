...
def construire_grille_edt(attributions, creneaux_liste):
    ...
    for attr in attributions:
        ...
        grille[cle_jour][creneau].append({'matiere': attr.get('matiere', ''), 'enseignant': attr.get('enseignant', ''),
            'lieu': attr.get('lieu', ''), 'surveillants': surv_text, 'promotion': attr.get('promotion', '')})
...
def generer_excel_edt(df_grille, promotion):
    ...
    for row in ws.iter_rows(min_row=2, max_row=ws.max_row):
        ...
        ws.row_dimensions[row[0].row].height = max(60, max_lines * 15)
...
def generer_html_edt(df_grille, promotion):
    ...
    <style>
        ...
        .edt-table td { padding: 8px; border: 1px solid #90CAF9; vertical-align: top; min-width: 200px; }
        ...
        .matiere { font-weight: bold; color: #1565C0; font-size: 12px; }
        .ens { color: #333; font-size: 10px; }
        .lieu { color: #E65100; font-size: 10px; font-weight: bold; }
        .surv { color: #2E7D32; font-size: 10px; }
...
def generer_pdf_edt(attributions, promotion, creneaux_liste):
    ...
    for _, row in df_grille.iterrows():
        ...
        for jour in jours_cols:
            val = row.get(jour, '')
            if val:
                val = val.replace('📖 ', '').replace('👤 ', 'Prof: ').replace('🏫 ', 'Lieu: ').replace('👮\n', 'Surv: ').replace('• ', '- ').replace('\n---\n', '\n\n')
                row_data.append(val)
            else:
                row_data.append('')
        ...
        for jour in jours:
            ...
            if creneau in planning_par_jour.get(jour, {}):
                for examen in planning_par_jour[jour][creneau]:
                    ...
                    html += f"<div class='examen-cell'><strong>{examen.get('matiere', '')}</strong><br><small>Promo: {examen.get('promotion', '')} | {examen.get('lieu', '')}</small><br><small>{surv_html}</small></div>"
            html += "</td>"
        html += "</tr>"
    html += "</table>"
    return html
...
def generer_excel_colore(attributions):
    ...
    for attr in attributions:
        ...
        data.append({'Date': date_str, 'Jour': jour, 'Creneau': attr.get('creneau', ''), 'Matiere': attr.get('matiere', ''), 'Promotion': attr.get('promotion', ''), 'Lieu': attr.get('lieu', ''), 'Surveillants': surv_str})
...
def generer_pdf(attributions):
    ...
    for attr in attributions:
        ...
        if hasattr(date_val, 'strftime'):
            ...
            jour = ''
        surv_str = ", ".join([f"{s['nom']} ({s['qualite']})" for s in attr.get('details_surveillants', [])])
        table_data.append([date_str, jour, attr.get('creneau', ''), attr.get('matiere', ''), attr.get('promotion', ''), attr.get('lieu', ''), surv_str])
...
def main():
    ...
    with tabs[2]:
        ...
        if st.session_state.data_loaded and st.session_state.promotions_list:
            ...
            if promo_selected:
                ...
                # ORDRE DES MATIERES
                st.markdown("---")
                st.markdown("#### 📋 Ordre des Matieres")
                st.markdown("<div class='info-box'>Definissez l'ordre des matieres (1= premiere, etc.)</div>", unsafe_allow_html=True)
                ...
                if not promo_examens.empty:
                    ...
                    edited_ordre = st.data_editor(ordre_df, column_config={"matiere": st.column_config.TextColumn("Matiere", disabled=True), "enseignant": st.column_config.TextColumn("Enseignant", disabled=True), "Ordre": st.column_config.NumberColumn("Ordre (1= premiere)", min_value=1, max_value=999, step=1)}, use_container_width=True, num_rows="fixed", key=f"ordre_editor_{promo_selected}")
                ...
                # HORAIRES PAR MATIERE
                st.markdown("---")
                st.markdown("#### 🕐 Horaires par Matiere (Optionnel)")
                st.markdown("<div class='info-box'>Choisissez un creneau specifique pour chaque matiere. Laissez 'Auto' pour la repartition automatique.</div>", unsafe_allow_html=True)
                if not promo_examens.empty:
                    ...
                    if promo_selected in st.session_state.horaires_par_matiere:
                        ...
                        horaire_df['Creneau'] = horaire_df['matiere'].map(hmap).fillna("Auto")
                    edited_horaire = st.data_editor(horaire_df, column_config={"matiere": st.column_config.TextColumn("Matiere", disabled=True), "enseignant": st.column_config.TextColumn("Enseignant", disabled=True), "Creneau": st.column_config.SelectboxColumn("Creneau", options=["Auto"] + CRENEAUX)}, use_container_width=True, num_rows="fixed", key=f"horaire_editor_{promo_selected}")
                    hmap = {}
                    for _, row in edited_horaire.iterrows():
                        if row['Creneau'] != "Auto": hmap[row['matiere']] = row['Creneau']
                    st.session_state.horaires_par_matiere[promo_selected] = hmap
                    nb_manuel = len(hmap)
                    if nb_manuel > 0: st.markdown(f"<div class='success-box'>✅ {nb_manuel} matiere(s) avec creneau manuel defini</div>", unsafe_allow_html=True)
                ...
                if st.session_state.planning_df is not None:
                    ...
                    if not planning_display.empty:
                        edited = st.data_editor(planning_display, column_config={"date": st.column_config.DateColumn("Date", format="DD/MM/YYYY"), "creneau": st.column_config.SelectboxColumn("Creneau", options=CRENEAUX), "lieu": st.column_config.SelectboxColumn("Lieu", options=SALLES + AMPHIS), "matiere": st.column_config.TextColumn("Matiere", disabled=True), "promotion": st.column_config.TextColumn("Promotion", disabled=True), "enseignant": st.column_config.TextColumn("Enseignant", disabled=True)}, use_container_width=True, num_rows="fixed", key=f"plan_ed_{promo_selected}")
                        ...
                        for _, row in edited.iterrows():
                            ...
                            if d is not None and not pd.isna(d):
                                ...
                                cal_data.append({'Date': ds, 'Jour': jr, 'Creneau': row.get('creneau', ''), 'Matiere': row.get('matiere', ''), 'Lieu': row.get('lieu', ''), 'Enseignant': row.get('enseignant', '')})
        ...
        if st.session_state.planning_df is not None and st.session_state.enseignants_df is not None:
            ...
            if st.session_state.surveillance_df is not None:
                ...
                for attr in st.session_state.surveillance_df:
                    ...
                    for surv in attr.get('details_surveillants', []):
                        attr_data.append({'Date': ds, 'Creneau': attr.get('creneau', ''), 'Matiere': attr.get('matiere', ''), 'Promotion': attr.get('promotion', ''), 'Lieu': attr.get('lieu', ''), 'Surveillant': surv['nom'], 'Qualite': surv['qualite'], 'Role': 'Charge de matiere' if surv.get('priorite') == 'Charge de matiere' else surv['qualite']})
                if attr_data:
    ...
    with tabs[4]:
        ...
        st.markdown("<div class='info-box'>Cet onglet genere des Emplois du Temps en grille pour chaque promotion. Les <b>jours sont en horizontal</b> et les <b>creneaux horaires en vertical</b>. Chaque cellule contient: la matiere, l'enseignant charge, les surveillants et le lieu.</div>", unsafe_allow_html=True)

        if st.session_state.surveillance_df is not None:
            # Grouper les attributions par promotion
            ...
            if promos_dans_attr:
                        ...
                        if attr_promo:
                            ...
                            if df_grille is not None and not df_grille.empty:
                                ...
                                with st.expander(f"🎨 Apercu visuel EDT {promo}"):
                                    st.components.v1.html(html_edt, height=500, scrolling=True)
                ...
                # Telechargement global de tous les EDT en un seul fichier Excel
                st.markdown("---")
