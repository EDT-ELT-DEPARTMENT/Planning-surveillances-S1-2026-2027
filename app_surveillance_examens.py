...
def generer_planning_promo(examens_df, promotion, date_debut, jours_feries, creneaux, lieux, ordre_matieres=None, horaires_matiere=None, nb_lieux_matiere=None):
    ...
    # Recuperer preferences horaires et jours
    promo_df['creneau_pref'] = None
    promo_df['jour_pref'] = None
    promo_df['nb_lieux_matiere'] = 1

    if horaires_matiere and promotion in horaires_matiere:
        hmap = horaires_matiere[promotion]
        for idx in promo_df.index:
            mat = promo_df.at[idx, 'matiere']
            if mat in hmap:
                h = hmap[mat]
                if isinstance(h, dict):
                    promo_df.at[idx, 'creneau_pref'] = h.get('creneau')
                    jour_val = h.get('jour')
                    if jour_val is not None and not pd.isna(jour_val):
                        if hasattr(jour_val, 'date'):
                            jour_val = jour_val.date()
                        elif isinstance(jour_val, str):
                            try:
                                jour_val = datetime.strptime(jour_val, "%Y-%m-%d").date()
                            except:
                                jour_val = None
                        promo_df.at[idx, 'jour_pref'] = jour_val
                else:
                    promo_df.at[idx, 'creneau_pref'] = h

    if nb_lieux_matiere and promotion in nb_lieux_matiere:
        nbl_map = nb_lieux_matiere[promotion]
        for idx in promo_df.index:
            mat = promo_df.at[idx, 'matiere']
            if mat in nbl_map:
                try:
                    promo_df.at[idx, 'nb_lieux_matiere'] = max(1, int(nbl_map[mat]))
                except:
                    promo_df.at[idx, 'nb_lieux_matiere'] = 1
    ...
    for i in promo_df.index:
        ...
        jour_pref = promo_df.at[i, 'jour_pref']
        nb_lieux_m = min(int(promo_df.at[i, 'nb_lieux_matiere']), nb_lieux_total)

        if jour_pref is not None and not pd.isna(jour_pref):
            date_exam = jour_pref
        else:
        ...
        for g in range(nb_lieux_m):
            lieu = lieux[lieu_idx % nb_lieux_total]
            lieu_idx += 1
        ...
        if not (jour_pref is not None and not pd.isna(jour_pref)):
            idx_creneau += 1
...
def attribuer_surveillants(planning_df, enseignants_df, nb_par_lieu=2):
    ...
    for idx, examen in planning_df.iterrows():
        ...
        attributions.append({'date': date_examen, 'creneau': creneau_examen, 'matiere': matiere_examen,
            ...
            'enseignant': enseignant_matiere, 'groupe': groupe})

    return attributions, surveillants

# ============================================================
# FONCTIONS EDT GRILLE
# ============================================================
...
def construire_grille_edt(attributions, creneaux_liste):
    ...
    grille = {}
    jours_info = []  # (date_obj, cle_jour) pour tri chronologique
...
# ============================================================
# FONCTIONS EXPORT EXISTANTES (modifiees)
# ============================================================
...
def generer_excel_colore(attributions):
    ...
    for nom, info in sorted(ens_count.items(), key=lambda x: x[1]['count'], reverse=True):
        resume_data.append({'Enseignant': nom, 'Qualite': info['qualite'], 'Nombre': info['count'], 'Examens': "; ".join(info['examens'][:5]) + ("..." if len(info['examens']) > 5 else "")})
...
def main():
    ...
    with tabs[0]:
        ...
        <div class="info-box">
            ...
            <ul>
                ...
                <li>🕐 <b>Horaire et Jour personnalisables par matiere</b></li>
                <li>🏫 <b>Nombre de lieux configurables par matiere</b> (multi-salles)</li>
                <li>📅 EDT en grille par promotion (jours horizontal, creneaux vertical)</li>
                <li>🎯 Attribution intelligente : Charge de matiere dans 1 seul lieu, Permanent 1er, Vacataire 2e</li>
        ...
        if st.session_state.data_loaded:
            ...
            with st.expander("👁️ Apercu des Cours extraits"):
                ...
                else: st.info("Aucun cours a afficher.")
            ...
            if promo_selected:
                ...
                if st.session_state.planning_df is not None:
                    ...
                    if not planning_display.empty:
                        ...
                        }, use_container_width=True, num_rows="fixed", key=f"plan_ed_{promo_selected}",
                        disabled=["qualite_ens", "ordre"])
        ...
        if st.session_state.planning_df is not None and st.session_state.enseignants_df is not None:
            ...
            if st.session_state.surveillance_df is not None:
                ...
                for attr in st.session_state.surveillance_df:
                    ...
                    for surv in attr.get('details_surveillants', []):
                        attr_data.append({'Date': ds, 'Creneau': attr.get('creneau', ''), 'Matiere': attr.get('matiere', ''), 
    ...
    with tabs[4]:
        ...
        st.markdown("<div class='info-box'>Cet onglet genere des Emplois du Temps en grille pour chaque promotion. Les <b>jours sont en horizontal</b> (par ordre chronologique) et les <b>creneaux horaires en vertical</b>. Chaque cellule contient: la matiere, le chargé de matière, les surveillants (Permanent en 1er, Vacataire en 2e) et le lieu.</div>", unsafe_allow_html=True)
        ...
        if st.session_state.surveillance_df is not None:
            ...
            if promos_dans_attr:
                    ...
                    with st.container():
                        ...
                        if attr_promo:
                            ...
                            if df_grille is not None and not df_grille.empty:
                                ...
                                with c1:
                                    html_edt = generer_html_edt(df_grille, promo)
                                    st.download_button(f"⬇️ HTML - {promo}", html_edt, f"EDT_{promo}.html", "text/html", key=f"dl_edt_html_{promo}")
                                with c2:
                                    excel_edt = generer_excel_edt(df_grille, promo)
                                    st.download_button(f"⬇️ Excel - {promo}", excel_edt, f"EDT_{promo}.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key=f"dl_edt_xlsx_{promo}")
                                with c3:
                                    pdf_edt = generer_pdf_edt(attr_promo, promo, CRENEAUX)
                                    st.download_button(f"⬇️ PDF - {promo}", pdf_edt, f"EDT_{promo}.pdf", "application/pdf", key=f"dl_edt_pdf_{promo}")

                                with st.expander(f"🎨 Apercu visuel EDT {promo}"):
                                    st.components.v1.html(html_edt, height=600, scrolling=True)
                ...
                st.markdown("---")
                st.markdown("### 📦 Telechargement Global (toutes les promotions)")
                if st.button("📥 Generer un Excel avec tous les EDT", type="primary", key="btn_all_edt"):
                    with st.spinner("Generation du fichier global..."):
                        wb_global = Workbook()
                        wb_global.remove(wb_global.active)

                        for promo in promos_dans_attr:
                            attr_promo = [a for a in st.session_state.surveillance_df if a.get('promotion', '') == promo]
                            df_grille, _, _ = construire_grille_edt(attr_promo, CRENEAUX)
                            if df_grille is not None and not df_grille.empty:
                                ws = wb_global.create_sheet(title=f"EDT {promo[:20]}")
                                header_fill = PatternFill(start_color="1565C0", end_color="1565C0", fill_type="solid")
                                header_font = Font(color="FFFFFF", bold=True, size=11)
                                thin_border = Border(left=Side(style='thin', color='90CAF9'), right=Side(style='thin', color='90CAF9'), top=Side(style='thin', color='90CAF9'), bottom=Side(style='thin', color='90CAF9'))
                                headers = ['Creneau'] + [c for c in df_grille.columns if c != 'Creneau']
                                for c_idx, header in enumerate(headers, 1):
                                    cell = ws.cell(row=1, column=c_idx, value=header)
                                    cell.fill = header_fill
                        ...
                        wb_global.save(buffer_global)
                        buffer_global.seek(0)
                        st.download_button("⬇️ Telecharger le fichier Excel global", buffer_global, "EDT_Toutes_Promotions.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="dl_all_edt")
            else:
    ...
    with tabs[5]:
        ...
        if st.session_state.surveillance_df is not None:
            ...
            with col1:
                ...
                with st.expander("Apercu"): st.components.v1.html(html_content, height=600, scrolling=True)
            with col2:
                st.markdown("#### 📊 Excel")
                excel_buffer = generer_excel_colore(attributions)
                st.download_button("⬇️ Telecharger Excel", excel_buffer, "planning_surveillances.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="dl_xlsx")
            with col3:
                st.markdown("#### 📑 PDF")
