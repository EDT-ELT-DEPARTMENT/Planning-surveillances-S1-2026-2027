...
REPERTOIRE_EDT = "EDTs_Generes"
NB_PROMOTIONS_TOTAL = 23
...
def init_session_state():
    defaults = {
        ...
        'nb_lieux_par_matiere': {}, 'historique_edt': {}
    }
...
# --- Fonctions répertoire EDT ---
def get_repertoire_edt():
    rep = os.path.join(os.path.dirname(os.path.abspath(__file__)), REPERTOIRE_EDT)
    os.makedirs(rep, exist_ok=True)
    return rep

def sauvegarder_fichier_edt(contenu, nom_fichier, is_bytes=False):
    rep = get_repertoire_edt()
    chemin = os.path.join(rep, nom_fichier)
    mode = "wb" if is_bytes else "w"
    encoding = None if is_bytes else "utf-8"
    with open(chemin, mode, encoding=encoding) as f:
        if is_bytes:
            f.write(contenu)
        else:
            f.write(contenu)
...
def compter_edt_dans_repertoire():
    rep = get_repertoire_edt()
    if not os.path.exists(rep):
        return 0
    fichiers = [f for f in os.listdir(rep) if f.startswith("EDT_")]
    promos = set()
    for f in fichiers:
        base = f.replace("EDT_", "")
        parts = base.rsplit(".", 1)
...
def creer_zip_edt(format_filtre=None):
    rep = get_repertoire_edt()
    zip_buffer = io.BytesIO()
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        for f in os.listdir(rep):
            if f.startswith("EDT_"):
                if format_filtre is None or f.endswith(f".{format_filtre}"):
                    zf.write(os.path.join(rep, f), f)
    zip_buffer.seek(0)
    return zip_buffer

def effacer_historique_edt():
...
def charger_historique_edt():
    rep = get_repertoire_edt()
    if not os.path.exists(rep):
        return
    for f in os.listdir(rep):
        if f.startswith("EDT_") and f.endswith(".html"):
            base = f.replace("EDT_", "").rsplit(".", 1)[0]
            if base not in st.session_state.historique_edt:
                st.session_state.historique_edt[base] = {
                    'html': f"EDT_{base}.html",
                    'excel': f"EDT_{base}.xlsx",
                    'pdf': f"EDT_{base}.pdf",
                    'date_generation': "Recharge"
                }
...
def charger_fichier_source_auto():
    try:
        ...
        # Garantir toutes les colonnes attendues
        for col in ['matiere', 'promotion', 'enseignant', 'qualite_ens', 'date', 'creneau', 'lieu', 'ordre']:
...
def generer_planning_promo(examens_df, promotion, date_debut, jours_feries, creneaux, lieux, ordre_matieres=None, horaires_matiere=None, nb_lieux_matiere=None):
    if examens_df is None or examens_df.empty:
        return None
    if 'promotion' not in examens_df.columns:
        st.error("Colonne 'promotion' manquante dans les donnees d'examens.")
        return None
    promo_df = examens_df[examens_df['promotion'].astype(str).str.strip() == str(promotion).strip()].copy()
    if promo_df.empty:
        return None
    for col in ['matiere', 'enseignant', 'qualite_ens']:
    ...
    idx_creneau = 0
    lieu_idx = 0
    nb_lieux_total = len(lieux)
    planning_rows = []

    for i in promo_df.index:
        matiere = promo_df.at[i, 'matiere']
        enseignant = promo_df.at[i, 'enseignant']
        qualite = promo_df.at[i, 'qualite_ens']

        # Horaires manuels
        jour_pref = None
        creneau_pref = None
        if horaires_matiere and promotion in horaires_matiere and matiere in horaires_matiere[promotion]:
            h = horaires_matiere[promotion][matiere]
            if isinstance(h, dict):
                jour_pref = h.get('jour')
                creneau_pref = h.get('creneau')
            else:
                creneau_pref = h

        if jour_pref and not pd.isna(jour_pref):
            if isinstance(jour_pref, str):
                try:
                    jour_pref = datetime.strptime(jour_pref, "%Y-%m-%d").date()
                except:
                    jour_pref = None
            elif hasattr(jour_pref, 'date'):
                jour_pref = jour_pref.date()
            date_exam = jour_pref
        else:
            while not est_jour_travaille(date_courante, jours_feries):
                date_courante += timedelta(days=1)
            date_exam = date_courante

        if creneau_pref and creneau_pref in creneaux_dispo:
            creneau = creneau_pref
        else:
            creneau = creneaux_dispo[idx_creneau % nb_creneaux]

        # Nombre de lieux pour cette matiere
        nb_lieux = 1
        if nb_lieux_matiere and promotion in nb_lieux_matiere and matiere in nb_lieux_matiere[promotion]:
            try:
                nb_lieux = int(nb_lieux_matiere[promotion][matiere])
            except:
                nb_lieux = 1
        nb_lieux = max(1, min(nb_lieux, len(lieux)))

        for g in range(nb_lieux):
            lieu = lieux[lieu_idx % len(lieux)]
            lieu_idx += 1
            planning_rows.append({
                ...
                'creneau': creneau,
                'lieu': lieu,
                'ordre': promo_df.at[i, 'ordre'] if 'ordre' in promo_df.columns else 999,
                'groupe': g
            })

        if not (jour_pref and not pd.isna(jour_pref)):
            idx_creneau += 1
...
def attribuer_surveillants(planning_df, enseignants_df, nb_par_lieu=2):
    ...
    for idx, examen in planning_df.iterrows():
        ...
        # Chargé de matiere UNIQUEMENT pour le premier groupe (groupe 0)
        if groupe == 0 and enseignant_matiere and str(enseignant_matiere) not in ['nan', '', 'None']:
        ...
        attributions.append({'date': date_examen, 'creneau': creneau_examen, 'matiere': matiere_examen,
            'promotion': examen.get('promotion', ''), 'lieu': lieu_examen, 'groupe': groupe,
            'surveillants': [s['nom'] for s in liste_surveillants], 'details_surveillants': liste_surveillants,
            'enseignant': enseignant_matiere})
...
def construire_grille_edt(attributions, creneaux_liste):
    ...
    for attr in attributions:
        ...
        if cle_jour not in grille:
            grille[cle_jour] = {}
            jours_info.append((date_val, cle_jour))
        ...
        charge_matiere = attr.get('enseignant', '')

        # Retirer le charge de matiere de la liste des surveillants pour eviter doublon
        survs_sans_charge = [s for s in survs if s['nom'] != charge_matiere]

        # Trier : Permanent d'abord, puis Vacataire, puis Autre
        ordre_qualite = {'Permanent': 0, 'Vacataire': 1, 'Autre': 2}
        survs_sans_charge.sort(key=lambda s: ordre_qualite.get(s['qualite'], 3))

        surv_text = "\n".join([f"• {s['nom']} ({s['qualite']})" for s in survs_sans_charge])

        grille[cle_jour][creneau].append({
            'matiere': attr.get('matiere', ''),
            'enseignant': charge_matiere,
            'lieu': attr.get('lieu', ''),
            'surveillants': surv_text,
            'promotion': attr.get('promotion', ''),
            'charge_matiere': charge_matiere
        })

    # Tri chronologique des jours
    jours_info.sort(key=lambda x: x[0])
    jours_ordre = [j[1] for j in jours_info]
    ...
    for creneau in creneaux_utilises:
        ...
        for jour in jours_ordre:
            ...
            if exams:
                cellules = []
                for ex in exams:
                    cell_text = f"📖 {ex['matiere']}\n👤 Chargé de matière : {ex['charge_matiere']}\n🏫 {ex['lieu']}\n👮 Surveillants :\n{ex['surveillants']}"
...
def generer_html_edt(df_grille, promotion):
    ...
    html = """
    <style>
        .edt-table { border-collapse: collapse; width: 100%; font-family: 'Segoe UI', Arial, sans-serif; font-size: 13px; }
        .edt-table th { background-color: #1565C0; color: white; padding: 12px; text-align: center; border: 2px solid #0D47A1; font-size: 14px; font-weight: bold; }
        .edt-table td { padding: 10px; border: 1px solid #90CAF9; vertical-align: middle; text-align: center; min-width: 220px; }
        .creneau-cell { background-color: #E3F2FD; font-weight: bold; text-align: center; font-size: 14px; width: 130px; }
        .exam-cell { background-color: #FFF8E1; }
        .matiere { font-weight: bold; color: #1565C0; font-size: 14px; text-align: center; }
        .ens { color: #333; font-size: 12px; text-align: center; font-weight: 600; }
        .lieu { color: #E65100; font-size: 12px; font-weight: bold; text-align: center; }
        .surv { color: #2E7D32; font-size: 12px; text-align: center; }
        .sep { border-top: 2px dashed #ccc; margin: 6px 0; }
    </style>
    <h2 style="color:#1565C0; text-align:center; font-family:'Segoe UI',Arial,sans-serif;">EDT Examens - Promotion """ + promotion + """</h2>
            ...
            if val:
                ...
                for part in parts:
                    ...
                    for line in lines:
                        ...
                        elif line.startswith('👮'): formatted.append(f"<div class='surv' style='font-weight:bold;margin-top:4px;'>{line}</div>")
                        elif line.startswith('• '): formatted.append(f"<div class='surv'>{line}</div>")
                        else: formatted.append(f"<div style='text-align:center;'>{line}</div>")
...
def generer_pdf_edt(attributions, promotion, creneaux_liste):
    ...
    title_style = ParagraphStyle('CustomTitle', parent=styles['Heading1'], fontSize=18, textColor=colors.HexColor('#1565C0'), spaceAfter=15, alignment=1)
    elements.append(Paragraph(f"EDT EXAMENS - {promotion}", title_style))
    elements.append(Paragraph("Annee 2026-2027 - Semestre 1", styles['Normal']))
    ...
    for _, row in df_grille.iterrows():
        ...
        for jour in jours_cols:
            val = row.get(jour, '')
            if val:
                val = val.replace('📖 ', '').replace('👤 ', 'Charge: ').replace('🏫 ', 'Lieu: ').replace('👮 Surveillants :', 'Surv:').replace('• ', '- ').replace('\n---\n', '\n\n')
                row_data.append(val)
            else:
                row_data.append('')
    ...
    col_widths = [3.5 * cm] + [(available_width - 3.5 * cm) / len(jours_cols)] * len(jours_cols)
...
def generer_tableau_html(attributions):
    ...
    <style>
        ...
        .charge-info { font-size: 9px; color: #555; margin-bottom: 3px; }
    </style>
        ...
        for jour in jours:
            ...
            if creneau in planning_par_jour.get(jour, {}):
                for examen in planning_par_jour[jour][creneau]:
                    ...
                    charge = examen.get('enseignant', '')
                    groupe = examen.get('groupe', 0)
                    lieu = examen.get('lieu', '')
                    surv_html = "<br>".join([f"<span class='surv-{s['qualite'].lower() if s.get('priorite') != 'Charge de matiere' else 'charge'}'>{s['nom']} ({s['qualite']}{'*' if s.get('priorite') == 'Charge de matiere' else ''})</span>" for s in survs])
                    html += f"<div class='examen-cell'><strong>{examen.get('matiere', '')}</strong><br><small>Promo: {examen.get('promotion', '')} | {lieu}{' (Groupe '+str(groupe+1)+')' if groupe>0 else ''}</small><br><div class='charge-info'>👤 Charge: {charge}</div><small>{surv_html}</small></div>"
            html += "</td>"
        html += "</tr>"
    html += "</table>"
    return html
...
def generer_excel_colore(attributions):
    ...
    for attr in attributions:
        ...
        data.append({
            'Date': date_str, 'Jour': jour, 'Creneau': attr.get('creneau', ''),
            'Matiere': attr.get('matiere', ''), 'Promotion': attr.get('promotion', ''),
            'Lieu': attr.get('lieu', ''), 'Charge': attr.get('enseignant', ''),
            'Groupe': attr.get('groupe', 0)+1, 'Surveillants': surv_str
        })
    ...
    for attr in attributions:
        for surv in attr.get('details_surveillants', []):
            nom = surv['nom']
            if nom not in ens_count: ens_count[nom] = {'count': 0, 'qualite': surv['qualite'], 'examens': []}
    ...
    for nom, info in sorted(ens_count.items(), key=lambda x: x[1]['count'], reverse=True):
        resume_data.append({
            'Enseignant': nom, 'Qualite': info['qualite'], 'Nombre': info['count'],
...
def generer_pdf(attributions):
    ...
    elements.append(Spacer(1, 0.3*cm))
    table_data = [['Date', 'Jour', 'Creneau', 'Matiere', 'Promotion', 'Lieu', 'Charge', 'Surveillants']]
    for attr in attributions:
        date_val = attr.get('date', None)
        if hasattr(date_val, 'strftime'):
            date_str = date_val.strftime('%d/%m/%Y')
            ...
            jour = ''
        surv_str = ", ".join([f"{s['nom']} ({s['qualite']})" for s in attr.get('details_surveillants', [])])
        table_data.append([date_str, jour, attr.get('creneau', ''), attr.get('matiere', ''), attr.get('promotion', ''), attr.get('lieu', ''), attr.get('enseignant', ''), surv_str])
...
def main():
    ...
    with st.sidebar:
        ...
        # --- AFFICHEUR NUMERIQUE EDT ---
        st.markdown("---")
        st.markdown("### 📁 EDTs Generes")
        charger_historique_edt()
        nb_edt_gen = len(st.session_state.historique_edt)
        pct = int(nb_edt_gen * 100 / NB_PROMOTIONS_TOTAL) if NB_PROMOTIONS_TOTAL > 0 else 0
        st.markdown(f"""
        <div style="background: linear-gradient(135deg, #1a237e 0%, #283593 100%); 
                    border-radius: 15px; padding: 15px; text-align: center; 
                    border: 3px solid #ffd700; box-shadow: 0 4px 15px rgba(0,0,0,0.3); margin-bottom: 10px;">
            <div style="font-size: 0.8rem; color: #90caf9; text-transform: uppercase; letter-spacing: 2px;">
                📅 EDT Generes
            </div>
            <div style="font-size: 2.5rem; font-weight: bold; color: #ffd700; font-family: 'Courier New', monospace;">
                {nb_edt_gen}<span style="font-size: 1.2rem; color: #90caf9;"> / {NB_PROMOTIONS_TOTAL}</span>
            </div>
            <div style="font-size: 0.7rem; color: #64b5f6;">
                {pct}% complete
            </div>
        </div>
        """, unsafe_allow_html=True)

        if st.button("🗑️ Effacer l'historique", key="btn_clear_hist", use_container_width=True):
            effacer_historique_edt()
            st.success("✅ Historique efface!")
            time.sleep(0.5)
            st.rerun()

        if st.session_state.historique_edt:
            with st.expander("📂 EDTs sauvegardes"):
                for promo, infos in sorted(st.session_state.historique_edt.items()):
                    st.caption(f"🎓 {promo} — {infos.get('date_generation', '')}")
        else:
            st.info("Aucun EDT genere")
    ...
    with tabs[0]:
        ...
        <div class="info-box">
            ...
            <p><strong>S1 - 2026-2027</strong> | Filtre: <b>Cours uniquement</b></p>
            <ul>
                ...
                <li>👥 Permanents / Vacataires + Exclusion manuelle</li>
                <li>🕐 <b>Jour et Horaire personnalisables par matiere</b></li>
                <li>🏫 <b>Nombre de lieux par matiere configurable</b></li>
                <li>📅 EDT en grille par promotion (jours horizontal, creneaux vertical)</li>
                <li>🎯 Attribution intelligente (Charge de matiere = 1 seul lieu)</li>
                <li>📊 Export HTML, Excel, PDF</li>
            </ul>
            <p><small>Vendredi et Samedi exclus. Dimanche travaillable.</small></p>
        </div>
        """, unsafe_allow_html=True)
        if st.session_state.data_loaded:
            ...
            with st.expander("👁️ Apercu des Cours extraits"):
                df_ex = st.session_state.examens_df
                expected_cols = ['matiere', 'promotion', 'enseignant']
                available_cols = [c for c in expected_cols if c in df_ex.columns]
                if available_cols and not df_ex.empty:
                    preview_df = df_ex[available_cols].head(20)
                    st.dataframe(preview_df, use_container_width=True, hide_index=True)
                else:
                    st.warning(f"Colonnes attendues non trouvees. Disponibles: {list(df_ex.columns)}")
    ...
    with tabs[1]:
        ...
        if st.session_state.data_loaded:
            ...
            exclus = st.multiselect("Selectionner les enseignants a EXCLURE", sorted(all_ens), default=st.session_state.exclus_manuels, key="w_exclus", help="0 surveillance pour ces enseignants")
    ...
    with tabs[2]:
        ...
        if st.session_state.data_loaded and st.session_state.promotions_list:
            promo_selected = st.selectbox("📚 Selectionner une promotion", st.session_state.promotions_list, index=0, key="w_promo_sel")
            ...
            if promo_selected:
                ...
                with col2:
                    ...
                    salles_sel = st.multiselect("Salles", SALLES, default=['S01', 'S02', 'S03', 'S04', 'S05'], key=f"w_sal_{promo_selected}")
                with col3:
                    st.markdown("#### 🎓 Amphis A01-A12")
                    amphis_sel = st.multiselect("Amphis", AMPHIS, default=['A01', 'A02', 'A03'], key=f"w_amp_{promo_selected}")
                ...
                st.markdown(f"<div class='info-box'>📊 <b>{nb_examens}</b> cours | Lieux: <b>{len(lieux_sel)}</b> selectionnes</div>", unsafe_allow_html=True)

                # ORDRE DES MATIERES + NB LIEUX
                st.markdown("---")
                st.markdown("#### 📋 Ordre des Matieres et Nombre de Lieux")
                st.markdown("<div class='info-box'>Definissez l'ordre des matieres (1= premiere) et le nombre de lieux simultanes pour chaque matiere.</div>", unsafe_allow_html=True)
                ...
                if not promo_examens.empty:
                    ...
                    if promo_selected in st.session_state.nb_lieux_par_matiere:
                        nbl_map = st.session_state.nb_lieux_par_matiere[promo_selected]
                        ordre_df['Nb_Lieux'] = ordre_df['matiere'].map(nbl_map).fillna(1).astype(int)
                    ordre_df = ordre_df.sort_values('Ordre')
                    edited_ordre = st.data_editor(ordre_df, column_config={
                        ...
                        "Nb_Lieux": st.column_config.NumberColumn("Nb Lieux", min_value=1, max_value=10, step=1, help="Nombre de salles/amphis simultanes pour cette matiere")
                    }, use_container_width=True, num_rows="fixed", key=f"ordre_editor_{promo_selected}")
                    edited_ordre = edited_ordre.sort_values('Ordre')
                    st.session_state.ordre_matieres[promo_selected] = edited_ordre['matiere'].tolist()
                    st.session_state.nb_lieux_par_matiere[promo_selected] = dict(zip(edited_ordre['matiere'], edited_ordre['Nb_Lieux']))
                    st.markdown("<div class='success-box'>✅ Ordre: " + " → ".join(edited_ordre['matiere'].tolist()[:5]) + ("..." if len(edited_ordre) > 5 else "") + "</div>", unsafe_allow_html=True)

                # HORAIRES ET JOUR PAR MATIERE
                st.markdown("---")
                st.markdown("#### 🕐 Jour et Horaire par Matiere (Optionnel)")
                st.markdown("<div class='info-box'>Choisissez un <b>jour</b> et un <b>creneau</b> specifiques pour chaque matiere. Laissez le jour vide et 'Auto' pour la repartition automatique.</div>", unsafe_allow_html=True)
                if not promo_examens.empty:
                    horaire_df = promo_examens[['matiere', 'enseignant']].copy()
                    horaire_df['Jour'] = pd.Series([None]*len(horaire_df), dtype='object')
                    horaire_df['Creneau'] = "Auto"
                    if promo_selected in st.session_state.horaires_par_matiere:
                        hmap = st.session_state.horaires_par_matiere[promo_selected]
                        for idx in horaire_df.index:
                            mat = horaire_df.at[idx, 'matiere']
                            if mat in hmap:
                                h = hmap[mat]
                                if isinstance(h, dict):
                                    if h.get('jour') is not None:
                                        horaire_df.at[idx, 'Jour'] = h['jour']
                                    if h.get('creneau') is not None:
                                        horaire_df.at[idx, 'Creneau'] = h['creneau']
                                else:
                                    horaire_df.at[idx, 'Creneau'] = h
                    edited_horaire = st.data_editor(horaire_df, column_config={
                        "matiere": st.column_config.TextColumn("Matiere", disabled=True),
                        "enseignant": st.column_config.TextColumn("Enseignant", disabled=True),
                        "Jour": st.column_config.DateColumn("Jour", format="DD/MM/YYYY", help="Date specifique de l'examen"),
                        "Creneau": st.column_config.SelectboxColumn("Creneau", options=["Auto"] + CRENEAUX)
                    }, use_container_width=True, num_rows="fixed", key=f"horaire_editor_{promo_selected}")
                    hmap = {}
                    for _, row in edited_horaire.iterrows():
                        entry = {}
                        jour = row['Jour']
                        if jour is not None and not pd.isna(jour):
                            ...
                            entry['jour'] = jour
                        if row['Creneau'] != "Auto":
                            entry['creneau'] = row['Creneau']
                        if entry:
                            hmap[row['matiere']] = entry
                    st.session_state.horaires_par_matiere[promo_selected] = hmap
                    nb_manuel = len(hmap)
                    if nb_manuel > 0: st.markdown(f"<div class='success-box'>✅ {nb_manuel} matiere(s) avec jour/creneau manuel defini</div>", unsafe_allow_html=True)

                # Bouton generation
                st.markdown("---")
                if st.button(f"🚀 Generer le Planning pour {promo_selected}", type="primary", key=f"btn_gen_{promo_selected}"):
                    if len(lieux_sel) == 0: st.error("❌ Selectionnez au moins un lieu.")
                    elif len(creneaux_sel) == 0: st.error("❌ Selectionnez au moins un creneau.")
                    else:
                        with st.spinner(f"Generation pour {promo_selected}..."):
                            ...
                            nb_lieux = st.session_state.nb_lieux_par_matiere.get(promo_selected, None)
                            planning = generer_planning_promo(st.session_state.examens_df.copy(), promo_selected, st.session_state.date_debut_val, st.session_state.jours_feries, creneaux_sel, lieux_sel, ordre, horaires, nb_lieux)
                            if planning is not None:
                                st.session_state.planning_df = planning
                                st.success(f"✅ Planning genere pour {promo_selected}! {len(planning)} seances planifiees.")
                            else: st.error("❌ Erreur de generation.")
                ...
                if st.session_state.planning_df is not None:
                    ...
                    if not planning_display.empty:
                        edited = st.data_editor(planning_display, column_config={
                            ...
                            "enseignant": st.column_config.TextColumn("Enseignant", disabled=True),
                            "groupe": st.column_config.NumberColumn("Groupe", disabled=True, help="0=1er lieu, 1=2e lieu, etc."),
                            "qualite_ens": None,
                            "ordre": None
                        }, use_container_width=True, num_rows="fixed", key=f"plan_ed_{promo_selected}", disabled=["qualite_ens", "ordre"])
                        for idx in edited.index:
                            for col in ['date', 'creneau', 'lieu']:
                                if col in edited.columns: st.session_state.planning_df.at[idx, col] = edited.at[idx, col]
                        ...
                        for _, row in edited.iterrows():
                            ...
                            if d is not None and not pd.isna(d):
                                ...
                                grp = row.get('groupe', 0)
                                lieu = row.get('lieu', '')
                                cal_data.append({'Date': ds, 'Jour': jr, 'Creneau': row.get('creneau', ''), 'Matiere': row.get('matiere', ''), 'Lieu': lieu, 'Groupe': grp+1, 'Enseignant': row.get('enseignant', '')})
                        if cal_data: st.dataframe(pd.DataFrame(cal_data), use_container_width=True, hide_index=True)
                    else: st.info("Aucun examen pour cette promotion.")
        else: st.warning("Aucune promotion detectee.")
    ...
    with tabs[3]:
        ...
        if st.session_state.planning_df is not None and st.session_state.enseignants_df is not None:
            if st.session_state.exclus_manuels: st.markdown(f"<div class='warning-box'>🚫 Exclus: {', '.join(st.session_state.exclus_manuels)}</div>", unsafe_allow_html=True)
            ...
            if st.session_state.surveillance_df is not None:
                ...
                for attr in st.session_state.surveillance_df:
                    ...
                    grp = attr.get('groupe', 0)
                    for surv in attr.get('details_surveillants', []):
                        attr_data.append({
                            'Date': ds, 'Creneau': attr.get('creneau', ''), 'Matiere': attr.get('matiere', ''),
                            'Promotion': attr.get('promotion', ''), 'Lieu': attr.get('lieu', ''), 'Groupe': grp+1,
                            'Surveillant': surv['nom'], 'Qualite': surv['qualite'],
                            'Role': 'Charge de matiere' if surv.get('priorite') == 'Charge de matiere' else surv['qualite']
                        })
                if attr_data:
                    attr_df = pd.DataFrame(attr_data)
                    def color_q(val):
                        cmap = {'Permanent': 'background-color: #E3F2FD', 'Vacataire': 'background-color: #E8F5E9', 'Autre': 'background-color: #FFF3E0', 'Charge de matiere': 'background-color: #F3E5F5'}
                        return cmap.get(val, '')
                    try: styled = attr_df.style.map(color_q, subset=['Qualite'])
                    except AttributeError: styled = attr_df.style.applymap(color_q, subset=['Qualite'])
                    st.dataframe(styled, use_container_width=True, hide_index=True)
                else: st.info("Aucune attribution a afficher.")
                ...
                for attr in st.session_state.surveillance_df:
                    for surv in attr.get('details_surveillants', []):
                        nom = surv['nom']
                        if nom not in ens_seances: ens_seances[nom] = []
                        d = attr.get('date', None)
                ...
                for nom, seances in ens_seances.items():
                    ...
                    for s in seances:
                        k = (s['date'], s['creneau'])
                        if k in seen: chevauchements.append({'Enseignant': nom, 'Date': str(s['date']), 'Creneau': s['creneau']})
                        seen[k] = True
                if chevauchements:
                ...
                else: st.success("✅ Aucun chevauchement!")
                ...
                for attr in st.session_state.surveillance_df:
                    for surv in attr.get('details_surveillants', []):
                        nom = surv['nom']
                        if nom not in resume: resume[nom] = {'count': 0, 'qualite': surv['qualite']}
                        resume[nom]['count'] += 1
                if resume:
                    resume_df = pd.DataFrame([{'Enseignant': k, 'Qualite': v['qualite'], 'Surveillances': v['count']} for k, v in sorted(resume.items(), key=lambda x: x[1]['count'], reverse=True)])
        ...
        else: st.warning("Generez d'abord le planning.")

    # ==================== EDT PAR PROMOTION ====================
    with tabs[4]:
        st.markdown('<div class="sub-header">📅 EDT en Grille par Promotion</div>', unsafe_allow_html=True)
        st.markdown("<div class='info-box'>Cet onglet genere des Emplois du Temps en grille pour chaque promotion. Les <b>jours sont en horizontal</b> (par ordre chronologique) et les <b>creneaux horaires en vertical</b>. Chaque cellule contient: la matiere, le charge de matiere, les surveillants (Permanent en 1er, Vacataire en 2e) et le lieu.</div>", unsafe_allow_html=True)

        if st.session_state.historique_edt:
            st.markdown("### 📦 Telechargement Groupé")
        ...
        if st.session_state.surveillance_df is not None:
            promos_dans_attr = set()
            for attr in st.session_state.surveillance_df:
                promos_dans_attr.add(attr.get('promotion', ''))
            promos_dans_attr = sorted([p for p in promos_dans_attr if p])

            if promos_dans_attr:
                st.markdown("### 📥 Telechargement des EDT par Promotion")
                ...
                for promo in promos_dans_attr:
                    with st.container():
                        ...
                        if attr_promo:
                            df_grille, jours_ordre, _ = construire_grille_edt(attr_promo, CRENEAUX)

                            if df_grille is not None and not df_grille.empty:
                                with st.expander(f"👁️ Apercu EDT {promo}"):
                                    st.dataframe(df_grille, use_container_width=True, hide_index=True)

                                # Génération des contenus
                                html_content = generer_html_edt(df_grille, promo)
                                excel_buffer = generer_excel_edt(df_grille, promo)
                                pdf_buffer = generer_pdf_edt(attr_promo, promo, CRENEAUX)

                                # Sauvegarde physique dans le répertoire
                                nom_html = f"EDT_{promo}.html"
                                nom_excel = f"EDT_{promo}.xlsx"
                                nom_pdf = f"EDT_{promo}.pdf"
                                ...
                                st.session_state.historique_edt[promo] = {
                                    'html': nom_html,
                                    'excel': nom_excel,
                                    'pdf': nom_pdf,
                                ...
                                with c1: st.download_button("⬇️ HTML", html_content, nom_html, "text/html", key=f"dl_edt_html_{promo}")
                                with c2: st.download_button("⬇️ Excel", excel_buffer, nom_excel, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key=f"dl_edt_xlsx_{promo}")
                                with c3: st.download_button("⬇️ PDF", pdf_buffer, nom_pdf, "application/pdf", key=f"dl_edt_pdf_{promo}")

                                with st.expander(f"🎨 Apercu visuel EDT {promo}"):
                                    st.components.v1.html(html_content, height=600, scrolling=True)

                                st.success(f"✅ EDT de **{promo}** sauvegarde dans `{REPERTOIRE_EDT}/`")
                                st.markdown("---")
                            else:
                                st.info(f"Aucune donnee pour generer l'EDT de {promo}")
                        else:
                            st.info(f"Aucune attribution pour {promo}")
            else:
                st.info("Aucune promotion trouvee dans les attributions.")
        else:
            st.warning("⚠️ Veuillez d'abord generer les attributions dans l'onglet '🎯 Attributions'.")
    ...
    with tabs[5]:
        ...
        if st.session_state.surveillance_df is not None:
            ...
            c1, c2, c3 = st.columns(3)
            with c1: st.download_button("⬇️ HTML", generer_tableau_html(attributions), "planning_surveillances.html", "text/html", key="dl_html")
            with c2: st.download_button("⬇️ Excel", generer_excel_colore(attributions), "planning_surveillances.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", key="dl_xlsx")
            with c3: st.download_button("⬇️ PDF", generer_pdf(attributions), "planning_surveillances.pdf", "application/pdf", key="dl_pdf")

            if st.session_state.historique_edt:
                st.markdown("---")
                st.markdown("### 📦 Telechargement Groupé des EDT")
                cg1, cg2, cg3, cg4 = st.columns(4)
                with cg1:
                    zip_all = creer_zip_edt()
        ...
        else: st.warning("Aucune attribution a exporter.")
